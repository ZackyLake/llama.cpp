...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nrows * nwarps) {
        ...
        const block_ptq1_0 * bxi = (const block_ptq1_0 *)(x + i * stride) + kbx0 + kbx;
#if defined(TURING_MMA_AVAILABLE)
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * rows_per_warp) {
        ...
        const block_ptq1_0 * bxi = (const block_ptq1_0 *)(x + i * stride) + kbx0 + scale_block;
#if defined(TURING_MMA_AVAILABLE)
        x_df[i * sram_stride + ksx] = bxi->d;
#else
        x_df[i * (2 * MMQ_TILE_NE_K / QI8_0) + i / (QI8_0 / 2) + ksx] = bxi->d;
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_load_tiles_iq4_kss(
    ...
    constexpr int threads_per_row = MMQ_TILE_NE_K / 4;
    constexpr int rows_per_warp   = warp_size / threads_per_row;
    const int kqsx = threadIdx.x % threads_per_row;
...
#ifdef __CUDA_ARCH__
    #pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps*(warp_size/16)) {
        int i = i0 + threadIdx.y*(warp_size/16) + threadIdx.x/16;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * (warp_size/4)) {
        int i = i0 + threadIdx.y * (warp_size/4) + threadIdx.x / 4;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += (nwarps * warp_size) / MMQ_TILE_NE_K) {
        ...
        const int grid = iq1s_grid_gpu[bxi->qs[4*kqsx+ir] | (((bxi->qh[ir] >> 3*kqsx) & 7) << 8)];

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
        grid32[1] = ((grid >> 4) & 0x0f0f0f0f) << 3;
        grid32[0] = (grid & 0x0f0f0f0f) << 3;
...
#else
        grid32[1] = (grid >> 4) & 0x0f0f0f0f;
        grid32[0] = grid & 0x0f0f0f0f;
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + 8*kbx + 2*kqsx + 0] = grid32[0];
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + 8*kbx + 2*kqsx + 1] = grid32[1];
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }
...
#else
        const int qh = bxi->qh[ir];
        const float d1q   = __half2float(dptr[ir]) * (((qh >> 11) & 0x0E) + 1);
        const float delta = -1.0f + IQ1S_DELTA - (qh & 0x8000) * (2.0f*IQ1S_DELTA/0x8000);
        x_ds[i*(MMQ_TILE_NE_K/4) + i/4 + kbxd] = make_half2(d1q, d1q*delta);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    }
}
