...
static void mul_mat_vec_iq1_s_r4_q8_1_cuda(const mmvq_args & args, cudaStream_t stream) {
    iqk_mul_mat_vec_q_cuda<GGML_TYPE_IQ1_S_R4, VDR_IQ1_S_R4_Q8_1_MMVQ, nullptr, 4, vec_dot_iq1_s_r4_q8_1>(args, stream);
}

static void mul_mat_vec_iq1_m_r4_q8_1_cuda(const mmvq_args & args, cudaStream_t stream) {
    iqk_mul_mat_vec_q_cuda<GGML_TYPE_IQ1_M_R4, VDR_IQ1_M_R4_Q8_1_MMVQ, nullptr, 4, vec_dot_iq1_m_r4_q8_1>(args, stream);
}
...
void ggml_cuda_op_mul_mat_vec_q_impl(ggml_backend_cuda_context & ctx, ggml_type type,
        ...
        const int64_t src1_padded_row_size, ggml_glu_op glu_op, float limit, cudaStream_t stream) {
...
void ggml_cuda_iqk_op_mul_mat_vec_q_3D(
    ...
    ggml_cuda_op_mul_mat_vec_q_impl(ctx, src0->type,
        ...
        src1_padded_row_size, GGML_GLU_OP_COUNT, 0.0f, stream);
...
void ggml_cuda_iqk_op_fused_mul_mat_vec_q_id(ggml_backend_cuda_context & ctx,
    ...
    if (bias_u) {
        GGML_ASSERT(bias_u->data);
        GGML_ASSERT(bias_u->ne[0] == dst->ne[0]);
    }
    if (bias_g) {
        GGML_ASSERT(bias_g->data);
    ...
    ggml_cuda_op_mul_mat_vec_q_impl(ctx, src0->type,
        ...
        row_low, row_high, src1_ncols,
        src1_padded_row_size, glu_op, limit, stream);
