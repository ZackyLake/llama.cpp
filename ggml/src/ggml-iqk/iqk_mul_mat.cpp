...
void MulMat::swiglu_clamp(int n, const float * x, float * y, float limit) {
    int i = 0;
#if defined __AVX512F__ && defined __AVX512DQ__
    const __m512 max512 = _mm512_set1_ps(limit);
    for (; i + 15 < n; i += 16) {
        const __m512 xc = v_clamp_max(_mm512_loadu_ps(x + i), max512);
        _mm512_storeu_ps(y + i, v_silu(xc));
    }
#endif
#if defined __AVX2__
    const __m256 max256 = _mm256_set1_ps(limit);
    for (; i + 7 < n; i += 8) {
        const __m256 xc = v_clamp_max(_mm256_loadu_ps(x + i), max256);
