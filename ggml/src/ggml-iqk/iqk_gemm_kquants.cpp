...
struct DequantizerIQ4XS final : public BaseDequantizer<block_iq4_xs> {
    DequantizerIQ4XS(const void * vx, size_t bx) : BaseDequantizer(vx, bx), values(load_iq4k_values_512()) {}
    template <typename Q8>
    inline void new_block(int i, const Q8&, __m256 *, __m512i * scales) {
        d = GGML_FP16_TO_FP32(x[i].d);
        prepare(x[i].qs);
...
static void mul_mat_iqX_k_q8_K_AVX512(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    for (int ix = 0; ix < nrc_x; ++ix) {
        ...
        for (int i = 0; i < nb; ++i) {
            ...
            for (int iy = 0; iy < nrc_y; ++iy) {
                const __m512i p1 = maddubs_signed_i8_512(deq.bits.values[0], q8.load_quants64(iy, i, 0));
                const __m512i p2 = maddubs_signed_i8_512(deq.bits.values[1], q8.load_quants64(iy, i, 1));
                const __m512i p3 = maddubs_signed_i8_512(deq.bits.values[2], q8.load_quants64(iy, i, 2));
                const __m512i p4 = maddubs_signed_i8_512(deq.bits.values[3], q8.load_quants64(iy, i, 3));
...
struct DequantizerIQ4XS final : public BaseDequantizer<block_iq4_xs> {
    DequantizerIQ4XS(const void * vx, size_t bx) : BaseDequantizer(vx, bx), values(_mm256_add_epi8(load_iq4nl_values_256(), _mm256_set1_epi8(-128))) {}
    template <typename Q8>
    inline __m256i new_block(int i, const Q8&, __m256 *) {
        d = GGML_FP16_TO_FP32(x[i].d);
        auto scales128 = siq4.make_scales(*(const uint32_t *)x[i].scales_l, x[i].scales_h);
        ...
        for (int i = 0; i < nb; ++i) {
            ...
            for (int j = 0; j < QK_K/128; ++j) {
                ...
                for (int iy = 0; iy < nrc_y; ++iy) {
                    ...
                    auto sumi4 = ggml_mm256_dpbusd_epi32(_mm256_setzero_si256(), deq.bits.values[3], _mm256_loadu_si256((const __m256i*)y.qs+3));
...
#else
    if constexpr (std::is_same_v<Dequantizer, DequantizerQ2K> || std::is_same_v<Dequantizer, DequantizerQ3K>) {
    ...
    for (int ix = 0; ix < nrc_x; ix += 16) {
        ...
        for (int ibl = 0; ibl < nbl; ++ibl) { // Block of 256
            ...
            for (int iy = 0; iy < nrc_y; ++iy) {
                ...
                acc[iy] = _mm512_fmadd_ps(m4, q8.sum_quants_512(iy, ibl), acc[iy]);
                isum[iy] = _mm512_setzero_si512();
            }
        }
        for (int iy = 0; iy < nrc_y; ++iy) {
...
bool iqk_set_kernels_kquants(int ne00, int typeA, int typeB, std::array<mul_mat_t, IQK_MAX_NY>& kernels, mul_mat_t& func16) {
    ...
    auto expected_type_B = etypeA == GGML_TYPE_Q4_K || etypeA == GGML_TYPE_Q5_K || etypeA == GGML_TYPE_Q6_K
                         ? GGML_TYPE_Q8_2_X4 : GGML_TYPE_Q8_K;
...
static void mul_mat_q8_KV_q8_KV_1(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    if (nrc_y == 1 && nrc_x == 1) { 
        ...
        for (int i = 2*(n/64); i < n/32; ++i) {
            ...
            isum256 = ggml_mm256_dpbusd_epi32(isum256, _mm256_add_epi8(qx, _mm256_set1_epi8(127)), qy); 
        }
    ...
    for (int ix = 0; ix < nrc_x; ++ix) {
        ...
        for (int i = 0; i < n/64; ++i) {
            ...
            for (int iy = 0; iy < nrc_y; ++iy) {
                for (int j = 0; j < 2; ++j) {
#ifdef HAVE_FANCY_SIMD
                    acc[2*iy+j] = ggml_mm256_dpbusd_epi32(acc[2*iy+j], qx[j], _mm256_loadu_si256((const __m256i *)q8y[iy] + 2*i + j));
#else
        ...
        if (int i = 2*(n/64); i < n/32) {
            ...
            for (int iy = 0; iy < nrc_y; ++iy) {
#ifdef HAVE_FANCY_SIMD
                acc[2*iy] = ggml_mm256_dpbusd_epi32(acc[2*iy], qx[0], _mm256_loadu_si256((const __m256i *)q8y[iy] + i));
#else
