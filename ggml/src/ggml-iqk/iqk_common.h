...
static inline float iqk_compute_fp16_to_fp32(ggml_fp16_t h) {
    __fp16 tmp;
    memcpy(&tmp, &h, sizeof(ggml_fp16_t));
    return (float) tmp;
}

static inline ggml_fp16_t iqk_compute_fp32_to_fp16(float f) {
    ggml_fp16_t res;
    __fp16 tmp = f;
...
#define GGML_FP16_TO_FP32(x) iqk_compute_fp16_to_fp32(x)
#define GGML_FP32_TO_FP16(x) iqk_compute_fp32_to_fp16(x)
...
enum {
    ...
    GGML_TYPE_Q8_0_R8  = 398,
    GGML_TYPE_Q8_K_R8  = 399,
};

static inline size_t iqk_packed_row_size(int type, int n) {
    switch (type) {
        case GGML_TYPE_Q8_0_R8:
            GGML_ASSERT(n % QK8_0 == 0);
            return (n/QK8_0)*sizeof(block_q8_0_r8)/8;
        case GGML_TYPE_Q8_K_R8:  return (n/QK_K)*sizeof(block_q8_k_r8)/8;
        case GGML_TYPE_Q8_K_R16: return (n/QK_K)*sizeof(block_q8_k_r16)/16;
        default: return 0;
    }
}
...
static inline __m512i load_iq4k_values_512() {
    return _mm512_broadcast_i32x4(load_iq4k_values_128());
}
#endif
...
template <int nrc, typename block_q8 = block_q8_K> struct Q8 {
    ...
    inline __m512 sum_quants_512(int iy, int i) const {
        // reduce the int16 bsums to the block total; the 128-bit reduce already
        // replicates the total in all 4 lanes, so convert and scale stay in the
        // 128-bit domain and a single 512-bit lane shuffle broadcasts the result
        const __m256i bsums = load_bsums(iy, i);                            // 16 x int16
        const __m256i m32 = _mm256_madd_epi16(bsums, _mm256_set1_epi16(1)); // 8 x int32
        const __m128i s0 = _mm_add_epi32(_mm256_castsi256_si128(m32), _mm256_extracti128_si256(m32, 1));
        const __m128i s1 = _mm_add_epi32(s0, _mm_shuffle_epi32(s0, _MM_SHUFFLE(2, 3, 0, 1)));
        const __m128i s2 = _mm_add_epi32(s1, _mm_shuffle_epi32(s1, _MM_SHUFFLE(1, 0, 3, 2))); // total in all 4 lanes
        const __m128 f  = _mm_mul_ps(_mm_cvtepi32_ps(s2), _mm_set1_ps(scale(iy, i)));          // d*total in all 4 lanes
        return _mm512_shuffle_f32x4(_mm512_castps128_ps512(f), _mm512_castps128_ps512(f), 0x00);
    }
#endif
...
#ifdef HAVE_AVX512_BW
static inline __m512i maddubs_signed_i8_512(__m512i values, __m512i q8) {
    const __m512i  zero      = _mm512_setzero_si512();
    const __mmask64 negative = _mm512_movepi8_mask(values);
    const __m512i  magnitude = _mm512_abs_epi8(values);
    const __m512i  signed_q8 = _mm512_mask_sub_epi8(q8, negative, zero, q8);
    return _mm512_maddubs_epi16(magnitude, signed_q8);
}
#endif
...
struct Q2Bits {
    inline void prepare(const uint8_t * q2) {
        ...
        const auto p0 = _mm512_shuffle_i64x2(q2bits, tmp, _MM_SHUFFLE(1, 0, 1, 0));
        const auto p2 = _mm512_shuffle_i64x2(q2bits, tmp, _MM_SHUFFLE(3, 2, 3, 2));
        values[0] = _mm512_and_si512(p0, ml);
        values[1] = _mm512_and_si512(_mm512_srli_epi16(p0, 4), ml);
        values[2] = _mm512_and_si512(p2, ml);
        values[3] = _mm512_and_si512(_mm512_srli_epi16(p2, 4), ml);
