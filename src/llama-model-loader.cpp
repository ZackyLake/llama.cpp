...
const char * llama_ftype_name(llama_ftype ftype) {
    ...
    switch ((enum llama_ftype) (ftype & ~LLAMA_FTYPE_GUESSED)) {
        ...
        case LLAMA_FTYPE_MOSTLY_IQ1_S_R4:  name = LLAMA_FTYPE_PREFIX "IQ1_S_R4 - 1.5 bpw"; break;
        case LLAMA_FTYPE_MOSTLY_IQ1_M_R4:  name = LLAMA_FTYPE_PREFIX "IQ1_M_R4 - 1.75 bpw"; break;
...
const void * llama_model_loader::load_data_range(const llama_tensor_weight & w, size_t offs, size_t size, void * buf) const {
    ...
    if (use_mmap) {
        ...
        file->read_raw_at(buf, w.offs + offs, size);
    }
